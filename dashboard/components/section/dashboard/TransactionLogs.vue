...
const testTransactions = ref<LogItem<Transaction>[]>([
	{
		id: '1',
		atmID: '000001',
		to: '104-000000-000',
	...
	},
	{
		...
		type: 'Withdrawal',
		sessionID: '1',
		timeCreated: Timestamp.fromDate(new Date('Feb 2, 2023 01:00:00')),
		timeCompleted: Timestamp.fromDate(new Date('Feb 2, 2023 01:01:00')),
		amount: 1000,
		status: 'Pending',
	},
	{
...
const statusesToSelect = ref(status_data.value.map(({ status }) => status))

const handleStatusSelection = (selection: any[]) => statusesToSelect.value = selection.map(({ status }) => status)
// #endregion

const filteredTransactions = computed(() => {
	// testTransactions.value.forEach(({ id, status }) => console.log(id, status))
	return testTransactions.value.filter(({ status }) => statusesToSelect.value.includes(status))
})
// #endregion
onMounted(() => {
	setTimeout(() => {
	...
	<SectionDashboardLogs>
		<template #filters>
			...
			<div class="inline-flex justify-end w-full">
				<div class="min-w-max">
					<FormMultiSelect
						class="border dark:border-white/20 rounded-md p-1"
						title="Status"
						...
						@option-selected="handleStatusSelection"
					/>
				</div>
			</div>
		</template>
		...
		<ul v-else class="flex flex-col w-full min-h-[calc(100vh-435px)] text-xl font-medium gap-2">
			<LogTransactionItem v-for="transaction in filteredTransactions" :key="transaction.id" :transaction="transaction" />
